    ...
    public void onEndPortalImpact(ChangeBlockEvent.Place event,@First Player player,@Root FallingBlock fallingblock) {
    	for (Transaction<BlockSnapshot> transaction : event.getTransactions()) {
			...
			if (type.equals(BlockTypes.CRAFTING_TABLE)){
				...
				player.playSound(SoundTypes.EXPLODE, SLL, 2);
				player.playSound(SoundTypes.FIREWORK_BLAST, SLL, 2);
				player.playSound(SoundTypes.FIREWORK_BLAST2, SLL, 2);
				
				player.spawnParticles(ParticleEffect.builder().type(ParticleTypes.EXPLOSION_NORMAL).build(), SLL);
				player.spawnParticles(ParticleEffect.builder().type(ParticleTypes.EXPLOSION_HUGE).build(), SLL);
				player.spawnParticles(ParticleEffect.builder().type(ParticleTypes.EXPLOSION_LARGE).build(), SLL);
