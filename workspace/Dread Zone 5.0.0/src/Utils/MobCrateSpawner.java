...
public class MobCrateSpawner {
	...
	public static void spawnMobDropper(Location<World> spawnLocation, int height){
		...
		Entity entityOptional = extent.createEntity(EntityTypes.FALLING_BLOCK, location.toDouble().add(0.5, 5+height, 0.5));
	    
	    FallingBlock block = (FallingBlock) entityOptional;
            ...
            extent.spawnEntity(block, Cause.of(NamedCause.source(SpawnCause.builder().type(SpawnTypes.PLUGIN).build())));
