...
public class TDMConfigUtils {
	...
	static List<Text> Teams = new ArrayList<>(Arrays.asList(Team1, Team2, Team3, Team4, Team5, Team6));
	
	static Text teamAName = getRandomTeam();
	
	static Text teamBName = getRandomTeamB(teamAName);
	...
	private static synchronized void teleportPlayersToArena(Object arenaName, List<Player> teamContestants, String teamName) {
		...
		for(Object arenaSP:arenaTeamSpawnNames){
			...
			for(Player player: teamContestants){
				...
				player.setLocationAndRotation(ContestantConfigUtils.sendContestantToArenaLocation(arenaName, teamName, arenaSP).add(0.5,0,0.5), 
	...
	public static void setUpTDMGUI(List<Player> teamContestants, Text teamName) {
		...
		for(Player player: teamContestants){
	        ...
	        Objective teamTagNameObjective = Objective.builder().name("Team")
	        		.displayName(Text.of(teamName))
			...
			//setup kills deaths scoreboard
		    Scoreboard scoreboard = Scoreboard.builder().build();
		    Objective objective = Objective.builder().name("Stats").displayName(
		    		Text.of(player.getName()+" Stats")).criterion(Criteria.DUMMY).build();//alter after we figure out how to get team color
		    ...
		    //Alter the tab list for the TDM
		    ...
		    player.getTabList().getEntry(player.getUniqueId()).get().setDisplayName(Text.of(player.getName()));//alter after we figure out how to get team color
		    ...
		    //setup the bossbar for the teams
